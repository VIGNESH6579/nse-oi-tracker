...
class SignalRepository:
    ...
    def tracking_for_date(self, trade_date: str, *, limit: int = 500) -> list[dict[str, Any]]:
        """Return one current tracking row per symbol/direction setup.

        The history endpoint intentionally retains each accepted setup.  The live
        tracking view should not make the operator scan repeated observations, so
        it selects the newest row for each symbol/direction and exposes only the
        fields needed to monitor the position.
        """
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT * FROM (
                    SELECT e.*, ROW_NUMBER() OVER (
                        PARTITION BY symbol, direction ORDER BY id DESC
                    ) AS rn
                    FROM signal_events e
                    WHERE trade_date = ? AND archived = 0
                )
                WHERE rn = 1
                ORDER BY CASE WHEN status IN ('OPEN', 'TG1_HIT') THEN 0 ELSE 1 END,
                         COALESCE(last_seen_at_ist, captured_at_ist) DESC
                LIMIT ?
                """,
                (trade_date, limit),
            ).fetchall()
        ...
        for row in rows:
            ...
            current = float(row["current_price"] or row["exit_price"] or entry)
            stop = float(row["stop_loss"] or 0)
            risk = abs(entry - stop) or entry * 0.003
            direction = str(row["direction"])
            signed_move = current - entry if direction == "BUY" else entry - current
            status = str(row["status"])
            tracked.append({
                "id": int(row["id"]),
                ...
                "status": status,
                "status_label": {
                    "OPEN": "MONITORING", "TG1_HIT": "TG1 HIT",
                    "TG2_HIT": "TG2 HIT", "SL_HIT": "STOP LOSS",
                    "BE_EXIT": "BREAKEVEN EXIT", "EXPIRED": "TIME EXIT",
                }.get(status, status),
                "result": row["result"],
                "result_r": row["result_r"],
                "live_r": round(signed_move / risk, 3) if risk else 0.0,
                "max_target_hit": row["max_target_hit"],
                "captured_at_ist": row["captured_at_ist"],
                "last_seen_at_ist": row["last_seen_at_ist"],
                "closed_at_ist": row["closed_at_ist"],
                "result_source": row["result_source"],
                "realtime_source": payload.get("realtime_source"),
                "oi_window": payload.get("oi_window"),
                "intraday_context": payload.get("intraday_context"),
